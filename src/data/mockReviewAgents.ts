...
try {
    ...
    debateStages: [
      {
        stage: 'independent_analysis',
        stageNumber: 1,
        stageTitle: '1. Independent Security Inspection',
        agentId: 'security',
        agentName: 'Security Guardian Agent',
        timestamp: '14:22:33',
        stance: 'flagged',
        argumentText: 'CRITICAL SECURITY DEFECT: Missing header presence check before invoking constructEvent cryptographic function. If stripe-signature is undefined, Stripe SDK throws unhandled TypeError before HMAC evaluation.',
        audioSpeechText: 'Security Guardian Agent analysis: Missing guard on stripe signature header in webhooks.ts.',
        evidenceCode: `event = stripeService.constructEvent(req.body, signature, webhookSecret);`,
        evidenceTokens: ['req.headers["stripe-signature"]', 'stripeService.constructEvent'],
      },
      {
        stage: 'cross_agent_challenge',
        stageNumber: 2,
        stageTitle: '2. Try/Catch Fallback Challenge',
        agentId: 'testing_reliability',
        agentName: 'Testing & Reliability Agent',
        timestamp: '14:22:36',
        stance: 'disagree_challenge',
        argumentText: 'Challenge: The invocation is already wrapped in a try/catch block. Does the TypeError not simply bubble up to the 400 response handler?',
        audioSpeechText: 'Testing agent challenge: The try catch block handles errors. Is an explicit signature guard strictly necessary?',
        confidenceDelta: -4,
      },
      {
        stage: 'debate_rebuttal',
        stageNumber: 3,
        stageTitle: '3. Security Protocol Rebuttal',
        agentId: 'security',
        agentName: 'Security Guardian Agent',
        timestamp: '14:22:38',
        stance: 'agree',
        argumentText: 'Rebuttal: Relying on library exception throwing creates noisy error telemetry and masks protocol-level malformed requests from automated probes. Explicit 400 Bad Request guards are required for cryptographic compliance.',
        audioSpeechText: 'Security agent rebuttal: Cryptographic compliance mandates upfront header validation before SDK evaluation.',
        confidenceDelta: +6,
      },
      {
        stage: 'evidence_verification',
        stageNumber: 4,
        stageTitle: '4. AST & Route Verification',
        agentId: 'code_quality_arch',
        agentName: 'Code Quality & Architecture Agent',
        timestamp: '14:22:39',
        stance: 'verified',
        argumentText: 'Verified AST in server/routes/webhooks.ts: Adding early return pattern `if (!signature) return res.status(400)` cleans up execution flow without side-effects.',
        audioSpeechText: 'Architecture agent verified: Early return pattern simplifies control flow cleanly.',
        confidenceDelta: +3,
      },
      {
        stage: 'consensus_ruling',
        ...
        timestamp: '14:22:40',
        stance: 'ruling',
        argumentText: 'Consensus Reached: High urgency blocker confirmed with 96% confidence. Webhook integrity guard approved for automated patching.',
        audioSpeechText: 'Orchestrator consensus: Verified critical webhook integrity guard with 96 percent confidence.',
      }
    ]
  },
  {
    ...
    agentsInvolved: ['testing_reliability', 'performance_db', 'code_quality_arch', 'security'],
    confidence: 91,
    ...
    debateStages: [
      {
        ...
        timestamp: '14:22:42',
        stance: 'flagged',
        argumentText: 'usePayment confirm hook does not generate unique client-side idempotency keys for retried requests. Flaky mobile networks risk duplicate confirmation POST requests.',
        audioSpeechText: 'Testing agent analysis: Missing client-side idempotency key on confirmation retry.',
        evidenceCode: `const res = await apiClient.post('/api/checkout/confirm', { paymentIntentId: clientSecret, billingDetails });`,
        evidenceTokens: ['paymentIntentId', 'confirmOrderHandler'],
      },
      {
        stage: 'cross_agent_challenge',
        stageNumber: 2,
        stageTitle: '2. Server-side Deduplication Check',
        agentId: 'performance_db',
        agentName: 'Performance & Database Agent',
        timestamp: '14:22:44',
        stance: 'disagree_challenge',
        argumentText: 'Challenge: Does the PostgreSQL `Order` table unique constraint on `stripePaymentIntentId` not already prevent duplicate records at the database tier?',
        audioSpeechText: 'Performance agent challenge: The database unique constraint already prevents duplicate records.',
        confidenceDelta: -3,
      },
      {
        stage: 'debate_rebuttal',
        stageNumber: 3,
        stageTitle: '3. UX & Retry Rebuttal',
        agentId: 'testing_reliability',
        agentName: 'Testing & Reliability Agent',
        timestamp: '14:22:45',
        stance: 'agree',
        argumentText: 'Rebuttal: The DB constraint throws a 500 server crash on duplicate insert rather than gracefully returning the already-created order. Client-generated idempotency keys allow deterministic cache lookups.',
        audioSpeechText: 'Testing agent rebuttal: DB constraint crashes with 500 instead of gracefully returning existing order.',
        confidenceDelta: +5,
      },
      {
        stage: 'evidence_verification',
        stageNumber: 4,
        stageTitle: '4. Branch Alignment Verification',
        agentId: 'git_merge',
        agentName: 'Git & Merge Intelligence Agent',
        timestamp: '14:22:46',
        stance: 'verified',
        argumentText: 'Verified branch history: This directly aligns with branch `fix/checkout-idempotency` and eliminates conflict during future merge.',
        audioSpeechText: 'Git merge agent verified: This aligns with branch intent and prevents future merge conflicts.',
      },
      {
        stage: 'consensus_ruling',
        stageNumber: 5,
        ...
        timestamp: '14:22:47',
        stance: 'ruling',
        argumentText: 'Consensus Reached: Medium severity accepted with 91% confidence. Client idempotency key patch approved.',
  ...
  },
  {
    ...
    agentsInvolved: ['code_quality_arch', 'security', 'testing_reliability'],
    confidence: 87,
    ...
    debateStages: [
      {
        ...
        evidenceCode: `billingDetails: { name: shippingAddress.fullName, address: shippingAddress }`,
      },
      {
        stage: 'cross_agent_challenge',
        stageNumber: 2,
        stageTitle: '2. Scope Pragmatism Challenge',
        agentId: 'performance_db',
        agentName: 'Performance & Database Agent',
        timestamp: '14:22:49',
        stance: 'disagree_challenge',
        argumentText: 'Challenge: Is introducing an explicit adapter layer over-engineering for a simple form payload?',
        audioSpeechText: 'Performance agent challenge: Is an adapter layer over-engineering for a simple form payload?',
      },
      {
        stage: 'debate_rebuttal',
        stageNumber: 3,
        stageTitle: '3. Domain Boundary Defense',
        agentId: 'code_quality_arch',
        agentName: 'Code Quality & Architecture Agent',
        timestamp: '14:22:50',
        stance: 'agree',
        argumentText: 'Rebuttal: ISO country codes and uppercase normalization prevent recurring Stripe API 400 invalid parameter rejections.',
        audioSpeechText: 'Architecture agent rebuttal: Normalization prevents Stripe parameter rejections at runtime.',
        confidenceDelta: +5,
      },
      {
        stage: 'evidence_verification',
        stageNumber: 4,
        stageTitle: '4. Form State Verification',
        agentId: 'testing_reliability',
        agentName: 'Testing & Reliability Agent',
        timestamp: '14:22:51',
        stance: 'verified',
        argumentText: 'Verified test assertions: Component unit test suite in `Checkout.test.tsx` passes cleanly with typed address fields.',
        audioSpeechText: 'Testing agent verified: Component tests pass with formatted address fields.',
      },
      {
        stage: 'consensus_ruling',
        stageNumber: 5,
